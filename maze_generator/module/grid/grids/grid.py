...
from maze_generator.module.cell.cells import (
    Cell,
    CellOver,  # Keep this. It's accessed in children
    CellUnder,  # Keep this. It's accessed in children
)
from maze_generator.module.constants import grid_logic as cst
from maze_generator.module.pathfinding.distance import Distances
...
class Grid:
    ...
    def calc_distances(self, props):
        if props.path.solution == "Random":
            ...
            start_cell = self[min(props.path.start[0], self.columns - 1), min(props.path.start[1], self.rows - 1)]
            end_cell = self[min(props.path.end[0], self.columns - 1), min(props.path.end[1], self.rows - 1)]
            setattr(self, "start_cell", start_cell)
            setattr(self, "end_cell", end_cell)
            self.distances = Distances(start_cell)
            self.longest_path = self.distances.path_to(end_cell).path
        else:  # Longest path possible
            distances = Distances(self.get_random_linked_cell(_seed=props.algorithm.seed))
