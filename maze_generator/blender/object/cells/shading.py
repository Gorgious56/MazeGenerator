...
import random
import bpy
from maze_generator.blender.shading.material.nodes import create_node, node_from_mat
from maze_generator.blender.mesh import constants as cst
from maze_generator.blender.driver.helper import setup_driver, DriverProperties, DriverVariable


def set_materials(scene, props):
    __set_material(props)
    __set_contour_material(props)
...
def __set_material(props) -> None:
    ...
    __create_nodes(props)
    __link_cell_nodes(props)


def __set_contour_material(props) -> None:
    obj_cells = props.objects.cells
    try:
...
def __create_nodes(props):
    ...
    create_node(
        ...
        (-1400, 0),
        attributes={"layer_name": cst.DISTANCE},
    )
    create_node(
        nodes,
        ...
        "ShaderNodeVertexColor",
        (-1400, 200),
        attributes={"layer_name": cst.NEIGHBORS},
    )
    create_node(
        nodes,
        ...
        "ShaderNodeVertexColor",
        (-1400, -200),
        attributes={"layer_name": cst.GROUP},
    )
...
def __link_cell_nodes(props):
    ...
    links.new(cr_distance.outputs[1], bsdf.inputs["Alpha"])
    links.new(mix_longest_distance.outputs[0], hsv.inputs[4])
    if props.display.paint_style == cst.DISTANCE:
        links.new(vertex_colors_distance.outputs[0], sep_rgb.inputs[0])
    elif props.display.paint_style == cst.UNIFORM:
        links.new(rgb.outputs[0], mix_longest_distance.inputs[1])
    elif props.display.paint_style == cst.GROUP:
