    ...
    def draw(self, context):
        ...
        if mg_props.grid:
            layout.label(text='Dead ends : ' +
                         str(mg_props.grid.dead_ends_amount), icon='CON_FOLLOWPATH')
            layout.label(text='Max Neighbors : ' +
                         str(mg_props.grid.max_links_per_cell), icon='LIBRARY_DATA_DIRECT')
            layout.label(
                text='Groups : ' + str(len(mg_props.grid.groups)), icon='SELECT_SUBTRACT')
