    ...
    def add_crossing(self, cell):
        ...
        if can_cross:
            ...
            if random() > 0.5:  # Vertical underway
                ...
                grid.link(north, north.neighbor(
                    cell.get_neighbor_return(cst.NORTH)))
                grid.link(south, south.neighbor(
                    cell.get_neighbor_return(cst.SOUTH)))
            else:
                ...
                grid.link(west, west.neighbor(
                    cell.get_neighbor_return(cst.WEST)))
                grid.link(east, east.neighbor(
                    cell.get_neighbor_return(cst.EAST)))
            return True
        return False
