...
class RecursiveVoronoiDivision(MazeAlgorithm):
    ...
    def __init__(self, grid, props=None, *args, **kwargs):
        ...
        # Destroy all walls:
        [[grid.link(c, n, False) for n in c.neighbors if n.level == c.level]
         for c in all_cells]
        self.run(all_cells)
    ...
    def run(self, cells):
        ...
        c_a, c_b = cells.pop(randrange(0, len(cells))), cells.pop(
            randrange(0, len(cells)))
        ...
        [(set_a if hypot(c.column - c_a.column, c.row - c_a.row) < hypot(c.column -
                                                                         c_b.column, c.row - c_b.row) else set_b).append(c) for c in cells]

        # Add the frontier walls to a container
        [[frontier.append((c_set_a, c_set_b)) for c_set_b in [
            _n for _n in c_set_a.neighbors if _n in set_b]] for c_set_a in set_a]
