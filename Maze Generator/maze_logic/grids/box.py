        ...
        for c in self.all_cells:
            ...
            if row == 2 * rows - 1:
                if col < rows:
                    c.set_neighbor(
                        cst.NORTH, 
                        self[rows, 3 * rows - col - 1, level], 
                        cst.WEST)
                elif rows + cols <= col < 2 * rows + cols:
                    c.set_neighbor(
                        cst.NORTH, self[rows + cols - 1, rows - cols + col, level], cst.EAST)
                elif col >= 2 * rows + cols:
                    c.set_neighbor(
                        cst.NORTH, self[3 * rows + 2 * cols - 1 - col, 3 * rows - 1, level], cst.NORTH)
            elif row == rows:
                if col < rows:
                    ...
                    c.set_neighbor(
                        cst.SOUTH, self[rows + cols - 1, 2 * rows + cols - 1 - col, level], cst.EAST)
                elif col >= 2 * rows + cols:
                    # Not working. Investigate
                    c.set_neighbor(
                        cst.SOUTH, self[3 * rows + 2 * cols - 1 - col, 0, level], cst.SOUTH)
