        ...
        if self[column, row, level] is None:
            ...
            if row % 3 == 0:  # Up triangle
                new_cell = Cell(
                    ...
                    corners=3,
                    half_neighbors=(1,))
                center = Vector(((2 if row % 2 == 0 else 1) +
                                 column * 2, 7 / 4 * (row / 3) + 1 / 4, 0))
                new_cell.first_vert_index = len(self.verts)
                self.verts.extend((Vector((size / 8, 0, 0)) + center, Vector(
                    (0, size / 4, 0)) + center, Vector((-size / 8, 0, 0)) + center))
